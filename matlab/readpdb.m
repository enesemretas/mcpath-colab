    ...
    if any(startsWith(lines, 'MODEL'))
        ...
        for i = 1:numel(lines)
            ...
            if startsWith(L, 'MODEL')
                if in_model
                    break; % second MODEL → stop reading
                end
                in_model = true;
                continue;
            elseif startsWith(L, 'ENDMDL') && in_model
                break;
            end
            if ~in_model
                % Some files have HEADER before MODEL → keep those lines too
                if startsWith(L, 'ATOM') || startsWith(L, 'HETATM') || startsWith(L, 'TER')
                    new_lines{end+1} = L;
                end
            else
                new_lines{end+1} = L;

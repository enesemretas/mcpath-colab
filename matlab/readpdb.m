...
% Octave-compatible version (logic/calculation unchanged).
% - Replaces 'startsWith' with strncmp
% - Replaces unique(...,'stable') with stable_unique()
% - Adds local_pdbread() fallback if 'pdbreader' is unavailable
% - Keeps all arrays/flows exactly as in your original code

y = 0;
if nargin < 2, chainID = []; end

% --- PREPROCESS PDB: limit to first MODEL if multi-model (e.g. NMR structures)
if exist(ProteinName, 'file')
    fid = fopen(ProteinName, 'r');
    C = textscan(fid, '%s', 'Delimiter', '\n', 'Whitespace', '');
    fclose(fid);
    lines = C{1};

    hasMODEL = any(cellfun(@(L) strncmp(L, 'MODEL', 5), lines));
    if hasMODEL
        fprintf('Detected multi-model PDB: trimming to first MODEL block.\n');
        new_lines = {};
        in_model = false;
        for i = 1:numel(lines)
            L = lines{i};
            if strncmp(L, 'MODEL', 5)
                if in_model
                    break; % second MODEL -> stop
                end
                in_model = true;
                continue;
            elseif strncmp(L, 'ENDMDL', 6) && in_model
                break;
            end
            if ~in_model
                % keep ATOM/HETATM/TER that appear before MODEL (rare)
                if strncmp(L,'ATOM',4) || strncmp(L,'HETATM',6) || strncmp(L,'TER',3)
                    new_lines{end+1} = L; %#ok<*AGROW>
                end
            else
                new_lines{end+1} = L;
            end
        end
        fid = fopen(ProteinName, 'w');
        for i = 1:numel(new_lines)
            fprintf(fid, '%s\n', new_lines{i});
        end
        fclose(fid);
        fprintf('Trimmed to first model successfully.\n');
    end
end

% --- Read PDB (with fallback)
if exist('pdbreader', 'file') == 2
    pdb = pdbreader(ProteinName);
else
    pdb = local_pdbread(ProteinName); % Octave-safe fallback
end

% Collect chain IDs from ATOM records (Octave-safe)
atoms = pdb.Model.Atom;
atomChains = arrayfun(@(a) upper(a.chainID), atoms, 'UniformOutput', false);
chains = stable_unique(atomChains);                        % preserve order
% drop blanks
keep_mask = ~cellfun(@isempty, cellfun(@strtrim, chains, 'UniformOutput', false));
chains = chains(keep_mask);
rank = [];
if ~isempty(chainID)
    % find (case-insensitive) first occurrence
    idx = find(strcmpi(chains, chainID), 1);
    if ~isempty(idx), rank = idx; end
end
...
while i~=ds
    i=i+1; j=j+1;
    % normalize some residue alt-names
    ...
    x(j,1)=x1(i,1); y(j,1)=y1(i,1); z(j,1)=z1(i,1);
    bfactor(j,1)=bfactor1(i,1); rfactor(j,1)=rfactor1(i,1);
    element{j}=element1{i};
end
ds=j;

% NEW: initialize so unknown/non-AA residues are marked 0
newrestype   = zeros(ds,1);   % 1..20 for amino acids, 0 for others
resatomcount = zeros(ds,1);
...
gly='GLY'; ala='ALA'; val='VAL'; ile='ILE'; leu='LEU';
ser='SER'; thr='THR'; asp='ASP'; asn='ASN'; glu='GLU';
gln='GLN'; lys='LYS'; arg='ARG'; cys='CYS'; met='MET';
phe='PHE'; tyr='TYR'; trp='TRP'; his='HIS'; pro='PRO';

atomcount=0; rescount=0;
glycount=1; alacount=1; valcount=1; ilecount=1; leucount=1;
sercount=1; thrcount=1; aspcount=1; asncount=1; glucount=1;
glncount=1; lyscount=1; argcount=1; cyscount=1; metcount=1;
phecount=1; tyrcount=1; trpcount=1; hiscount=1; procount=1;
...
for i=1:ds
    ...
    if strcmp(gly,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            glycount=glycount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(ala,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            alacount=alacount+1; rescount=rescount+1;
            resatomcount(rescount,1)=atomcount; res_num(i)=resnumber(i,1); atomcount=0;
        end
    end
    if strcmp(val,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            valcount=valcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(ile,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            ilecount=ilecount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(leu,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            leucount=leucount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(ser,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            sercount=sercount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(thr,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            thrcount=thrcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(asp,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            aspcount=aspcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(asn,restype{i})==1 || strcmp('ASH',restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            asncount=asncount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(glu,restype{i})==1 || strcmp('GLH',restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            glucount=glucount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(gln,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            glncount=glncount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(lys,restype{i})==1 || strcmp('LYN',restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            lyscount=lyscount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(arg,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            argcount=argcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(cys,restype{i})==1 || strcmp('CYM',restype{i})==1 || strcmp('CYX',restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            cyscount=cyscount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(met,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            metcount=metcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(phe,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            phecount=phecount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(tyr,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            tyrcount=tyrcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(trp,restype{i})==1
        ...
        if (i==ds || resnumber(i,1)~=resnumber(i+1,1))==1
            trpcount=trpcount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(his,restype{i})==1 || strcmp('HID',restype{i})==1 || strcmp('HIE',restype{i})==1 || strcmp('HIP',restype{i})==1
        ...
        if i==ds ||  resnumber(i,1)~=resnumber(i+1,1)
            hiscount=hiscount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end
    if strcmp(pro,restype{i})==1
        ...
        if i==ds ||  resnumber(i,1)~=resnumber(i+1,1)
            procount=procount+1; rescount=rescount+1;
            res_num(i)=resnumber(i,1); resatomcount(rescount,1)=atomcount; atomcount=0;
        end
    end

    % write Cα rows for standard amino acids (newrestype ~= 0)
...
glycount=0; alacount=0; valcount=0; ilecount=0; leucount=0;
sercount=0; thrcount=0; aspcount=0; asncount=0; glucount=0;
glncount=0; lyscount=0; argcount=0; cyscount=0; metcount=0;
phecount=0; tyrcount=0; trpcount=0; hiscount=0; procount=0;

m=size(output); totresno=m(1);
...
for i=1:totresno-1
    if output(i,1)==1
        if resatomcount(i,1)>1
            nonerr_index(k)=i; k=k+1;
        else
            err_index(l)=i;  erro(l,:)=[output(i,2) output(i,3)]; l=l+1;
        end
    end
    if output(i,1)==2
...
for i=1:totresno
    ...
    if output(i,1)==2
        if resatomcount(i,1)>4
            ...
            alacount=alacount+1;
            output(i,8)=output(i,5); output(i,9)=output(i,6); output(i,10)=output(i,7);
            output(i,11)=alainfo(alacount,1,4);
        end
    end
    if output(i,1)==3
...
h=size(output); resno=h(1);
k=0; b=0;
for i=1:ds-1
    b=b+1;
    if strcmp('N',atomtype{i+1})~=0 || strcmp(restype{i},restype{i+1})==0
        k=k+1; output(k,4)=b; b=0;
    end
end
output(k+1,4)=b+1;
...
% Lacking side-chain coordinates -> replace by backbone coordinates
for i=1:resno
...
if ~isempty(chainID)
    if isempty(rank)
        ...
        keep = (result(:,10) == rank);
        result = result(keep, :);
        % (Optional) normalize chain to 1:
        % result(:,10) = 1;
    end
end

% === Write output files (unchanged format) ===
File = strcat(ProteinName, '.cor');
fid=fopen(File,'w');
fprintf(fid, '%5.0f %4.0f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %4.0f %2.0f %4.1f\n', result');
fclose(fid);

if exist('erro','var') && ~isempty(erro)
    File = 'problem';
    fid = fopen(File,'w');
...
% Helpers (Octave-safe)
% -------------------------------------------------------------------------
function U = stable_unique(C)
% like unique(C,'stable') for cellstr C
U = {};
seen = containersMapLike();
for i=1:numel(C)
    key = C{i};
    if ~isKeyLike(seen, key)
        U{end+1} = key;
        putKeyLike(seen, key, true);
    end
end
end

function m = containersMapLike()
% Minimal string->logical map for Octave without containers.Map requirement
m.keys = {};
m.vals = [];
end

function tf = isKeyLike(m, key)
idx = find(strcmp(m.keys, key), 1);
tf = ~isempty(idx);
end

function putKeyLike(m, key, val)
idx = find(strcmp(m.keys, key), 1);
if isempty(idx)
    m.keys{end+1} = key;
    m.vals(end+1) = logical(val);
else
    m.vals(idx) = logical(val);
end
assignin('caller', inputname(1), m); % update struct by reference-like
end

function pdb = local_pdbread(fname)
% Fallback PDB reader that provides the subset of fields your code uses:
% AtomSerNo, AtomName, altLoc, resName, chainID, resSeq, iCode, X, Y, Z, element
% Parses standard PDB columns for ATOM/HETATM records.
fid = fopen(fname,'r');
...
while true
    ...
    if length(L) < 6, continue; end
    if strncmp(L,'ATOM  ',6) || strncmp(L,'HETATM',6)
        % Safe substring with bounds checking
        AtomSerNo = str2double(safesub(L,7,11));
        AtomName  = strtrim(safesub(L,13,16));
        altLoc    = strtrim(safesub(L,17,17));
        resName   = strtrim(safesub(L,18,20));
        chainID   = strtrim(safesub(L,22,22));
        resSeq    = str2double(safesub(L,23,26));
        iCode     = strtrim(safesub(L,27,27));
        X         = str2double(safesub(L,31,38));
        Y         = str2double(safesub(L,39,46));
        Z         = str2double(safesub(L,47,54));
        element   = strtrim(safesub(L,77,78));
        S.AtomSerNo = AtomSerNo;
        S.AtomName  = AtomName;
        S.altLoc    = altLoc;
        S.resName   = resName;
        S.chainID   = chainID;
        ...
        S.Z         = Z;
        S.element   = element;
        atoms = [atoms S]; %#ok<AGROW>
    end
end
fclose(fid);
pdb.Model.Atom = atoms;
end

function out = safesub(s,a,b)
a = max(1, a); b = min(length(s), b);
if a>b, out = ''; else, out = s(a:b); end
end

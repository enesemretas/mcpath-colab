...
# Optional: import strict Python parser if present (won't block UI if missing)
_rps = None
try:
    from . import readpdb_strict as _rps
except Exception:
    try:
        import readpdb_strict as _rps
    except Exception:
        _rps = None
...
def _list_or_custom(label: str, options, default_value, minv, maxv=None, step=1, **kwargs):
    if maxv is None and "max" in kwargs:  # back-compat
        maxv = kwargs["max"]
...
# -------------------- File upload helpers --------------------
def _fu_read_change_payload(new_payload):
    """
    Return (name, bytes) from ipywidgets v8 change['new'] or v7-like dict.
    """
    if isinstance(new_payload, dict) and new_payload:
        fname = next(iter(new_payload.keys()))
        meta = new_payload[fname]
        content = meta.get("content") if isinstance(meta, dict) else getattr(meta, "content", None)
        return fname, content
    if isinstance(new_payload, (tuple, list)) and new_payload:
        uf = new_payload[0]
...
def _fu_has_file_value(v):
    if isinstance(v, dict): return len(v) > 0
    if isinstance(v, (tuple, list)): return len(v) > 0
    return False

# -------------------- Optional local Python readpdb runner --------------------
def _maybe_run_local_readpdb(pdb_path: str, chain: str, enable: bool):
    if not enable or _rps is None:
        return
    try:
        runner = None
        for name in ("run", "run_readpdb", "readpdb"):
            if hasattr(_rps, name):
                runner = getattr(_rps, name); break
        if runner is None:
            print("ℹ️ readpdb_strict present but no callable run/readpdb; skipping.")
            return
...
def launch(
    ...
    # Manual Colab-upload fallback buffer
    manual_upload = {"name": None, "content": None}
    ...
    pdb_upload = W.FileUpload(
        accept=".pdb,.ent,.PDB,.ENT,.txt,.gz", multiple=False, description="Choose file"
    )

    # Update label from raw change payload (fixes v8)
    def _on_upload_change(change):
        try:
            fname, content = _fu_read_change_payload(change.get("new"))
            if fname and content:
                file_lbl.value = fname
            else:
                file_lbl.value = "No file chosen"
        except Exception:
            # fallback: inspect widget.value
            v = getattr(pdb_upload, "value", None)
            if _fu_has_file_value(v):
                fname, content = _fu_read_change_payload(v)
                file_lbl.value = fname or "1 file"
            else:
                file_lbl.value = "No file chosen"
    pdb_upload.observe(_on_upload_change, names="value")

    # Colab native upload fallback
    btn_colab_upload = W.Button(
        description="Use Colab Upload", icon="cloud-upload", button_style=""
    )
    def _colab_upload(_):
        try:
            from google.colab import files as gfiles
            picked = gfiles.upload()  # opens a file chooser
            if picked:
                name, content = next(iter(picked.items()))
                manual_upload["name"] = name
                manual_upload["content"] = content
                file_lbl.value = f"{name} (via Colab)"
        except Exception as e:
            file_lbl.value = f"Upload failed: {e}"
    btn_colab_upload.on_click(_colab_upload)

    upload_row = W.HBox([pdb_code, W.HTML("&nbsp;<b>or</b>&nbsp;"), pdb_upload, file_lbl, btn_colab_upload])
    ...
    def on_clear(_):
        ...
        manual_upload["name"] = None; manual_upload["content"] = None

        # Recreate the upload widget to truly reset it
        nonlocal pdb_upload, file_lbl
        pdb_upload = W.FileUpload(accept=".pdb,.ent,.PDB,.ENT,.txt,.gz", multiple=False, description="Choose file")
        file_lbl = W.Label("No file chosen")
        pdb_upload.observe(_on_upload_change, names="value")
        upload_row.children = [pdb_code, W.HTML("&nbsp;<b>or</b>&nbsp;"), pdb_upload, file_lbl, btn_colab_upload]

        with out: clear_output()

    def _collect_pdb_bytes():
        # 1) Manual Colab upload wins if present
        if manual_upload["name"] and manual_upload["content"]:
            return manual_upload["content"], manual_upload["name"]

        # 2) ipywidgets upload, read directly from current .value
        v = getattr(pdb_upload, "value", None)
        if _fu_has_file_value(v):
            fname, content = _fu_read_change_payload(v)
            if fname and content:
    ...
    def on_submit(_):
        with out:
            ...
            try:
                ...
                # Save local copy for optional Python parsing
                with open(pdb_name, "wb") as f:
                    f.write(pdb_bytes)
                print(f"Saved local copy: {os.getcwd()}/{pdb_name}")

                # Optional: run Python readpdb locally and preview .cor

# mcpath/ui.py — refined UI module
# - Stronger reset ("New Job") that reliably clears log + file upload
# - Safer path-length selectors with list/custom toggle
# - Button busy/disabled state during submit to avoid double posts
# - More robust resource path discovery (works in Colab where __file__ may be absent)
# - Clearer errors with tracebacks inside the shared log panel
...
def launch(
    ...
    MAX_FILE_MB = int(cfg.get("max_pdb_mb", 20))

    def _on_upload_change(_):
        if pdb_upload.value:
            name, meta = next(iter(pdb_upload.value.items()))
            size_ok = len(meta.get("content", b"")) <= MAX_FILE_MB * 1024 * 1024
            file_lbl.value = (name if size_ok else f"{name} (too large > {MAX_FILE_MB} MB)")
            if not size_ok:
                pdb_upload.value = {}
    ...
    row_np3, np3_ctrl = _list_or_custom_row("Number of Paths", num_paths_mode3, num_paths_mode3_default, 1, 10_000_000, 100)
    ...
    btn_clearlog = W.Button(description="Clear Log", button_style="warning", icon="eraser")
    ...
    body_children += [
        ...
        mode3_box,
        chain_id, email,
        W.HBox([btn_submit, btn_new_job, btn_clearlog]),
        W.HTML("<hr>"),
        out
    ]
    ...
    def _set_busy(is_busy: bool):
        btn_submit.disabled = is_busy
        btn_new_job.disabled = is_busy
        btn_clearlog.disabled = is_busy
        btn_submit.icon = "hourglass" if is_busy else "paper-plane"

    # --- New Job: reset everything on the existing form ---
    def on_new_job(_):
        try:
            # Reset simple text fields to config defaults
            ...
            file_lbl.value = "No file chosen"
            # ipywidgets quirk handling (works across versions)
            for attempt in ((), {}, None):
                try:
                    pdb_upload.value = attempt  # type: ignore
                    break
                except Exception:
                    pass
            ...
            # Reset list/custom rows to original default values
            ...
            with out:
                print("❌ Error while resetting UI:")
                traceback.print_exc()

    def _collect_pdb_bytes():
        if pdb_upload.value:
            (name, meta) = next(iter(pdb_upload.value.items()))
            return meta.get("content", b""), (meta.get("metadata", {}) or {}).get("name", name) or "upload.pdb"
    ...
    def on_submit(_):
        out.clear_output(wait=True)  # always clear the shared area first
        _set_busy(True)
        with out:
            try:
                ...
                if not _is_valid_email(email.value):
                ...
                if mode == "functional":
                    rows = [
                        "1", pdb_name, chain_global, str(get_big_len()), (email.value.strip() or "-")
                    ]
                elif mode == "paths_init_len":
                    if not _is_valid_chain(init_chain.value):
                        raise ValueError("Chain of initial residue must be a single character.")
                    rows = [
                        "2", pdb_name, chain_global, str(get_short_len()), str(get_num_paths_2()),
                        str(int(init_idx.value)), (init_chain.value or "").strip(), (email.value.strip() or "-")
                    ]
                else:  # paths_init_final
                    if not _is_valid_chain(init_chain.value):
                        raise ValueError("Chain of initial residue must be a single character.")
                    if not _is_valid_chain(final_chain.value):
                ...
                if run_readpdb is None:
                    ...
                    try:
                        # Prefer signature(input_path=...) when available
                        cor_path = run_readpdb(input_path=input_path)
                    except TypeError:
                        # Fallback to legacy signature(pdb_path, chain)
                ...
                # Attempt atomistic step if we have a COR
                try:
                    if cor_path and os.path.isfile(cor_path):
                        ...
                        # Resolve package resources even if __file__ is missing
                        here = os.path.dirname(getattr(sys.modules[__name__], "__file__", os.getcwd()))
                        ...
                        try:
                            ...
                            _ = atom_mod.atomistic(
                                base_for_atom, param_file=param_path, top_file=top_path,
                    ...
                    print("❌ Unexpected error in atomistic block:")
                    traceback.print_exc()
    ...
    btn_clearlog.on_click(on_clear_log)
